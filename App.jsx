...
// ── 消費予測の閾値（7日以内 = そろそろ切れる）─────────────────────
const SOON_DAYS = 7;

export default function App() {
  const [cats,      setCats]      = useState(null);
  const [items,     setItems]     = useState(null);
  const [activeTab, setActiveTab] = useState(null); // "soon" | カテゴリーID
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setCats(DEFAULT_CATEGORIES); setItems(DEFAULT_ITEMS);
  ...
  const addItem = (catId, d) => {
    const item = {
      ...
      stock: +d.stock, threshold: +d.threshold, unit: d.unit,
      history: [], usageLogs: [],
    };
  ...
  const updateItem = (catId, d) => {
    saveItems({
      ...items,
      [catId]: items[catId].map(i =>
        i.id === d.id ? { ...i, ...d, stock: +d.stock, threshold: +d.threshold } : i
      ),
    });
    setModal(null); toast_("更新しました");
  };
  ...
  // ── Phase3: 履歴から再登録 ───────────────────────────────────────
  // 過去の履歴をタップ → prefillとしてAddPurchaseModalを開く（入力なし再登録）
  const reuseHistory = (catId, item, histRecord) => {
    setModal({
      ...
      prefill: {
        ...
        brand:       histRecord.brand,
        description: histRecord.description,
        imageUrl:    histRecord.imageUrl,
        memo:        histRecord.memo || "",
        isFavorite:  histRecord.isFavorite || false,
      },
    });
  };
  ...
  const allItemsWithPred = cats.flatMap(cat =>
    (items[cat.id] || []).map(item => ({
      ...item,
      cat,
      pred: calcPrediction(item),
    }))
  );

  // 「そろそろ切れる」: 予測あり & daysLeft <= SOON_DAYS
  const soonItems = allItemsWithPred.filter(
    i => i.pred && i.pred.daysLeft <= SOON_DAYS
  ...
  return (
    <>
        ...
        <header className="header">
          <div className="header-top">
            ...
            <div className="header-right">
              <button className="add-cat-btn" onClick={() => setModal({ type: "addCat" })}>
                ＋ カテゴリー
              </button>
          ...
          <div className="tabs">
            {/* そろそろ切れるタブ（データがある時のみ表示）*/}
            {soonItems.length > 0 && (
              <button
                className={`tab alert ${activeTab === "soon" ? "active" : ""}`}
          ...
          {activeTab === "soon" && (
            activeItems.length === 0
              ...
              : activeItems.map(item => {
                  ...
                  return (
                    <div key={`${catId}-${item.id}`}
                      className={`item-card ${isLow ? "low" : "soon"}`}
                      ...
                      <div className="item-main">
                        <div className="item-info">
                          ...
                          {!isLow && <span className="soon-badge">⚠️ そろそろ</span>}
                        </div>
                      ...
                      <div className="ctrl-row" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="progress-bar">
                          <div className="progress-fill"
                            style={{ width: `${pct}%`, background: isLow ? "#E8734A" : "#D4A843" }} />
                        </div>
              ...
              {activeItems.length === 0 && !editMode
                ? (
                  <div className="empty">
                    ...
                    <div>アイテムがありません</div>
                    <div style={{ fontSize: 12, marginTop: 6 }}>下の「編集」から追加できます</div>
                  </div>
                )
                : activeItems.map(item => {
                    const isLow  = item.stock <= item.threshold;
                    ...
                    return (
                      <div key={item.id}
                        className={`item-card ${isLow ? "low" : isSoon ? "soon" : ""}`}
                        ...
                        {!editMode && (
                          <div className="ctrl-row" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="progress-bar">
                              <div className="progress-fill"
                                style={{ width: `${pct}%`, background: isLow ? "#E8734A" : isSoon ? "#D4A843" : activeCat?.color }} />
                            </div>
                        ...
                        {editMode && (
                          <div className="edit-row" onClick={e => e.stopPropagation()}>
                            <button className="edit-btn"
                              onClick={() => setModal({ type: "editItem", catId: activeTab, item: { ...item } })}>
                              ✏️ 編集
                            </button>
                            <button className="del-btn" onClick={() => deleteItem(activeTab, item.id)}>
                              🗑️ 削除
                            </button>
                          </div>
                        )}
                  ...
                  : <div className="edit-panel">
                      <div className="edit-panel-row">
                        <button className="btn-add-item"
                          onClick={() => setModal({ type: "addItem", catId: activeTab })}>
                          ＋ アイテムを追加
                        </button>
                        <button className="btn-cat-settings"
                          onClick={() => setModal({ type: "editCat", cat: { ...activeCat } })}>
                          ⚙️ 設定
                        </button>
                      </div>
      ...
      {modal && (
        <ModalLayer modal={modal} setModal={setModal} cats={cats}
